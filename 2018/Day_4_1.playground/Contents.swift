    ...
    for parsedAction in sortedActions {
        ...
        case .wakesUp(let minute):
            ...
            for min in fallsAsleepMinute..<minute {
                ...
                guardSleepRecords[currentGuard]?[min] = previousSleepLength + 1
            }

            guard let guardSleepRecord = guardSleepRecords[currentGuard] else { continue }

            let totalSleepTime = guardSleepRecord.reduce(0) {
                return $0 + $1.value
            }


            if totalSleepTime > maxMinutesAsleep {
                maxMinutesAsleep = totalSleepTime
                guardWhoSleptTheMost = currentGuard
            }
        }
    }

    guard let guardSleepRecord = guardSleepRecords[guardWhoSleptTheMost] else { return 0 }

    let max = guardSleepRecord.map({ $1 }).max()

    let minuteGuardSleptTheMost = guardSleepRecord.first { (key, value) -> Bool in
        return value == max
    }

    print("Maximum Number of Minutes Slept: \(maxMinutesAsleep)")
    print("Guard who slept the most: \(guardWhoSleptTheMost)")
    print("Minute when the guard was asleep the most: \(minuteGuardSleptTheMost?.key ?? 0)")

    return guardWhoSleptTheMost * (minuteGuardSleptTheMost?.key ?? 0)
}
