...
func findNonOverlappingClaim(for claimStrings: [String]) -> String {
    ...
    for claimString in claimStrings {
        ...
        claimants.insert(claim.claimant)
    }

    for (_, values) in claims {
        if values.count > 1 {
            for value in values {
                claimants.remove(value)
            }
        }
    }
...
class CodeTests: XCTestCase {
    ...
    func testExample1() {
        ...
        XCTAssertEqual(findNonOverlappingClaim(for: claimStrings), "3")
    }

    func testFindNonOverlappingClaim() {
        let claims = input()
        XCTAssertEqual(findNonOverlappingClaim(for: claims), "894")
    }
